...
predefined set of adducts with the `rules` parameter. Below we use only primary
adducts for positive polarity.
...
At last we extract the *edge list* from the `CAMERA` result object with the
...
associations between potential adducts/isotopes from the same *pcgroup* defined
by `CAMERA`) in each row. All edges fulfill the criteria from `CAMERA`
(i.e. representing signal from co-eluting ions with a similar peak shape), but
only for few the actual adduct annotation could be determined (see below).
...
appended to the feature annotations from the `xcms` result returned by
`featureDefinitions` and the feature abundance levels (extracted with
`featureValues`).

```{r message = FALSE}
quant_table <- cbind(
    as.data.frame(featureDefinitions(processedData)),
    getFeatureAnnotations(xsaFA),
    featureValues(processedData, value = "into"))
quant_table <- quant_table[, !(colnames(quant_table) %in% "peakidx")]

```

At last we export this edgelist to the file *camera_edgelist.csv* and the
feature annotation and quantification table to *camera_iin_quant_table*. These
can be used as input for the Ion Identity Networking in GNPS.

```{r}
write.csv(edgelist, file = "camera_iin_edgelist.csv", row.names = FALSE,
          quote = FALSE, na = "")
write.table(quant_table, file = "camera_iin_quant_table.txt",
            row.names = FALSE, quote = FALSE, sep = "\t", na = "")
```

In addition we subset both tables to features for which adduct annotations are
available.

```{r}
quant_table_sub <- quant_table[!is.na(quant_table[, "best ion"]), ]
edgelist_sub <- edgelist[!is.na(edgelist$Annotation), ]
write.csv(edgelist_sub, file = "camera_iin_edgelist_sub.csv", row.names = FALSE,
          quote = FALSE, na = "")
write.table(quant_table_sub, file = "camera_iin_quant_table_sub.txt",
            row.names = FALSE, quote = FALSE, sep = "\t", na = "")
```
