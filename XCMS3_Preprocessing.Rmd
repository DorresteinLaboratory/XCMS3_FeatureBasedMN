...
Note: This script is optimized for the latest R version (>3.6). Be sure to get the most recent version of R before running.  Note: You don't have to run this chunk if the packages are already installed.
...
Note: the CAMERA package supports objects of class xcmsSet, which were the outputs of the _old_ version of xcms.  
So we have to transform the XCMSnExp in xcmsSet trhough the command 'as(object, "xcmsSet", msLevel = 1L)'.  
Issue: For now, this command does not support objects of class MS2. A solution can be to perform a second preprocessing on the MS1 data before performing the CAMERA annotation. 
